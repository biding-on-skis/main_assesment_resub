...
if want_menu == "yes":
    ...
    print(f"You have ordered the following: {curry_order} with the sizes: {curry_order_size}")
    ...
    if want_menu == "yes":
        ...
        for drink_id in drink_order:
            drink_name, _ = drink_selector(drink_id, 'can')  # assuming default size for display
            print(f"- {drink_name}")

        # Update drink_size variable based on the last selected drink size
        if drink_order:
            drink_size = string_checker("What Size | Bottle (1.5L) | Can (330mL) |: ", size_drink)
